-- WORKING FINAL SETUP - VICTORIA FOOTBALL CLUB
-- Fixed for user permissions (no auth.users modifications)

-- Step 1: Drop public schema objects only (no auth.users access)
...
-- Step 2: Drop policies BEFORE dropping functions (correct order)
...
-- Step 4: Drop functions (after policies are gone)
...
-- SUCCESS: Victoria FC database setup complete!
-- Note: Auto-profile creation via auth.users trigger requires admin privileges
-- Users can still sign up and create profiles manually through the app
